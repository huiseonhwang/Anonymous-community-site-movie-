...
<h1 style="text-align: center;">
	<a href="adminMBoardDeleteForm.jsp">관리자용 영화게시판</a>
</h1>
...
<div id="center">
	<%
		if(count > 0){
			...
			if(startPage > 10){%>
				<a href="adminMSearch.jsp?pageNum=<%=startPage-10%>&colum=<%=colum%>&search=<%=search%>">[이전]</a>
			<%}
				for(int i = startPage ; i <= endPage ; i++){
				%>	<a href="adminMSearch.jsp?pageNum=<%=i%>&colum=<%=colum%>&search=<%=search%>">[<%=i%>]</a> 	
		  	  <%}
			if(endPage < pageCount){%>
			<a href="adminMSearch.jsp?pageNum=<%=startPage + 10%>&colum=<%=colum%>&search=<%=search%>">[다음]</a>
		  <%}	
		}
	%>
</div>
